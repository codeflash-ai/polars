...
def _is_iterable_of(val: Iterable[object], eltype: type | tuple[type, ...]) -> bool:
    ...
    # Optimize for builtin lists/tuples for small types
    if isinstance(val, (list, tuple)):
        # Fast path using a for loop (avoids generator overhead in all())
        for x in val:
...
def is_int_sequence(
    ...
    # Fastest possible path ordering: numpy first, then polars, then Sequence
    if _check_for_numpy(val) and isinstance(val, np.ndarray):
        return np.issubdtype(val.dtype, np.integer)
    if include_series and isinstance(val, pl.Series):
        return val.dtype.is_integer()
    # Cache this lookup, then check contents only if true
